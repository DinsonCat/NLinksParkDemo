package com.nlinks.parkdemo.widget.wheelpicker;
...
/*
 * 2017年03月10日 这个与项目中compile 'cn.aigestudio.wheelpicker:WheelPicker:1.0.0'
 * 属于用一个项目，但是由于1.0.0beta版和后期较完善版的很多方法和属性不同，旧版又涉及到过多页面，且考虑到页面以后可能会修改
 * 所以单独创建了这两个类，这两个类属于目前最新版本1.1.2。以后如果有更新，最好更新到最新版
 */
...
public class WheelPicker extends View implements IWheelPicker, Runnable {
    ...
    private boolean isPosInRang(int position) {
        return hasData() && position >= 0 && position < mData.size();
    }
    ...
    public void setSelectedItemPosition(int position) {
        if (hasData()) {
            ...
            //上面是我之前写的一个完美的计算不越界的方案，但我要改掉！2017年5月6日
	        if (position < 0 || position > mData.size() - 1) position = 0;
    ...
    public void setData(List data) {
        ...
        // 重置位置
        if (mSelectedItemPosition > mData.size() - 1 || mCurrentItemPosition > mData.size() - 1) {
            mSelectedItemPosition = mCurrentItemPosition = mData.size() - 1;
        } else {
    ...
    public void setMaximumWidthTextPosition(int position) {
        if (!isPosInRang(position))
            throw new ArrayIndexOutOfBoundsException("Maximum width text Position must in [0, mData.size()]" +
