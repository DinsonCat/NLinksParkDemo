    ...
    public void onGlobalLayout() {
        if (once) {
            ...
            if (d == null)
                return;
            // 垂直方向的边距
            mVerticalPadding = (getHeight() - (int) ((getWidth() - 2 * mHorizontalPadding) * mAspectRatio)) / 2;
...
//            initScale = (getWidth() * 1.0f - mHorizontalPadding * 2) / Math.min(dw, dh);

            float vScale = (getHeight() * 1.0f - 2 * mVerticalPadding) / dh;
            float hScale = (getWidth() * 1.0f - mHorizontalPadding * 2) / dw;
            initScale = Math.max(vScale, hScale);
